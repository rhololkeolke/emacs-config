...
#+begin_src emacs-lisp
  ...
  (defun cmake-ide-compile-target (target)
    ...
    (let ((old-cide--get-compile-command (symbol-function 'cide--get-compile-command)))
      (cl-letf (((symbol-function 'cide--get-compile-command)
                 (lambda (dir) (format "%s %s" (funcall old-cide--get-compile-command dir) target))))
        (cmake-ide-compile))))

  (defun cmake-ide-test ()
    (interactive)
    (cmake-ide-compile-target "test"))
  ...
  (use-package cmake-ide
    :config
    (add-hook 'c-mode-common-hook '(lambda () (local-set-key (kbd "<f5>") 'cmake-ide-compile)))
    (add-hook 'c-mode-common-hook '(lambda () (local-set-key (kbd "<f6>") 'cmake-ide-test)))
    (add-hook 'c-mode-common-hook '(lambda () (local-set-key (kbd "C-<f5>") 'cmake-ide-compile-target)))
    (add-hook 'c-mode-common-hook '(lambda () (local-set-key (kbd "<f7>") 'cmake-ide-compile-run))))
#+end_src
