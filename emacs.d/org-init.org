...
#+BEGIN_SRC emacs-lisp
    ...
    (add-hook 'after-make-frame-functions '(lambda (frame)
                                             (message "running after frame hook") (select-frame frame) (rhol--configure-font)))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (cond
   ((string= system-name "dschwab.lon.corp.google.com")
    (progn
      (setq org-directory (expand-file-name "~/gdrive/org-mode"))
      (setq org-agenda-files '("~/gdrive/org-mode"))))
   ((string= system-name "dschwab-macbookpro.roam.corp.google.com")
      (progn
        (setq org-directory (expand-file-name "~/Google Drive/org-mode/"))
        (setq org-agenda-files '("~/Google Drive/org-mode/"))))
   (t
    (progn
      (setq org-directory (expand-file-name "~/Dropbox/org-mode/"))
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq org-capture-templates
        '(("t" "todo" entry (file (lambda () (expand-file-name (format "%s/refile.org" org-directory))))
           "* TODO %?\n%U\n" :clock-in t :clock-resume t)
          ("r" "respond" entry (file (lambda () (expand-file-name (format "%s/refile.org" org-directory))))
           "* NEXT Respond to %:from on %:subject\nSCHEDULED: %t\n%U\n" :clock-in t :clock-resume t :immediate-finish t)
          ("n" "note" entry (file (lambda ()(expand-file-name (format "%s/refile.org" org-directory))))
           "* %? :NOTE:\n%U\n" :clock-in t :clock-resume t)
          ("j" "Journal" entry (file+datetree (lambda () (expand-file-name (format "~/%s/diary.org" org-directory))))
           "* %? :crypt:\n%U\n" :clock-in t :clock-resume t)
          ("e" "Event" entry (file (lambda () (expand-file-name (format "%s/calendar.org" org-directory))))
           "* %^{Name} \n%U\n:PROPERTIES:\n:CATEGORY: Event\n:LOCATION: %^{Location}\n:END:\n%^T\n\n%?")
          ("h" "Habit" entry (file (lambda () (expand-file-name (format "%s/refile.org" org-directory))))
           "* NEXT %?\n%U\nSCHEDULED: %(format-time-string \"<%Y-%m-%d %a .+1d/3d>\")\n:PROPERTIES:\n:STYLE: habit\n:REPEAT_TO_STATE: NEXT\n:END:\n")
          ("c" "Contacts" entry (file (lambda () (expand-file-name (format "%s/contacts.org" org-directory))))
           "* %^{Name} \n:PROPERTIES:\n:EMAIL: %^{Email}\n:PHONE:\n:NICKNAME:\n:IGNORE:\n:ICON:\n:NOTE:\n:ADDRESS:\n:BIRTHDAY:\n:END:\n%?")
          ("i" "Ideas" entry (file (lambda () (expand-file-name (format "%s/ideas.org" org-directory))))
           "* %?\n%U\n" :clock-in t :clock-resume t)
          ;; wait for an e-mail reply
          ("W" "wait-for-reply" entry (file+headline (lambda() (expand-file-name (format "%s/refile.org" org-directory))) "Waiting")
           "* WAIT %u %a %?\n")
          ))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package org-ref
    :config
    ;; org-ref settings
    (setq org-ref-notes-directory (format "%s/bibliography/notes" org-directory)
          org-ref-bibliography-notes (format "%s/bibliography/articles.org" org-directory)
          org-ref-default-bibliography '(lambda (_ _ _) (format "%s/bibliography/references.bib" org-directory))
          org-ref-pdf-directory (format "%s/bibliography/bibtex-pdfs" org-directory))
    ;; helm-bibtex settings
    (setq bibtex-completion-bibliography (format "%s/bibliography/references.bib" org-directory)
          bibtex-completion-library-path (format "%s/bibliography/bibtex-pdfs" org-directory)
          bibtex-completion-notes-path (format "%s/bibliography/articles.org" org-directory))
    ;; pdf opener
