...
#+begin_src emacs-lisp
  ...
  (defmacro do-once-secs-after-emacs-startup (time &rest body)
    `(run-with-idle-timer ,time ; run this after emacs is idle for time seconds
                          nil ; do this just once; don't repeat
                          (lambda () ,@body)))
#+end_src

#+results:
: do-once-secs-after-emacs-startup
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun rhol/setup-frame (&optional frame)
  ...
  "
    ...
    (when (window-system)
      (configure-font)))
  (if (daemonp)
      (add-hook 'after-make-frame-functions #'rhol/setup-frame)
    (rhol/setup-frame))
#+END_SRC
...
#+begin_src emacs-lisp
  ...
  ;;   :straight (el-patch :type git :host github :repo "lastquestion/explain-pause-mode")
  ;;   :config (explain-pause-mode nil))
#+end_src
...
*** jq-format
#+begin_src emacs-lisp
  (use-package jq-format :demand t :after json-mode)
#+end_src
...
#+begin_src emacs-lisp
  (use-package jsonnet-mode
    ...
    "jsonnetfmt"
    "Jsonnet command to run in ‘jsonnetfmt-reformat-buffer’."
    :type '(string)
    :group 'jsonnet)
    (defun jsonnet-reformat-buffer ()
    "Reformat entire buffer using the Jsonnet format utility."
    (interactive)
    (call-process-region (point-min) (point-max) jsonnetfmt-command t t nil "-")))
#+end_src
...
#+BEGIN_SRC emacs-lisp
  (use-package elpy
    ...
    :init (elpy-enable)
    :config
    ;; Override format to use black or print error
    ;;
    ;; existing function will try yapf and autopep8 too
    (defun elpy-format-code ()
      ...
      (cond
       ((elpy-config--package-available-p "black")
        (elpy-black-fix-code))
       (t
        (message "Install black to format code."))))
    )
...
#+begin_src emacs-lisp
  (defcustom lsp-pyls-plugins-flake8-max-line-length 90
  ...
  ;;                                '(("pyls.plugins.flake8.max-line-length" lsp-pyls-plugins-flake8-max-line-length t)))))
...
*** Set WORKON_HOME env var if not set

#+begin_src emacs-lisp
  (when (string-empty-p (getenv "WORKON_HOME"))
...
#+begin_src emacs-lisp
  ...
  ;; redefine rust--format-call to pass "--edition 2018" flags
...
** Gin Config
#+BEGIN_SRC emacs-lisp
  (message "org-init.org: gin-config")
  (use-package gin
    :load-path "~/.emacs.d/lisp"
    :mode ("\\.gin\\'" . gin-mode))
#+END_SRC
    ...
    :hook ((web-mode .
           ...
           (web-mode . (lambda ()
                         (when (string-equal "tsx" (file-name-extension buffer-file-name))
                           (setup-tide-mode)))))
    :config ;; configure jsx-tide checker to run after your default jsx checker
...
#+begin_src emacs-lisp
  ...
  (use-package switch-window
    :config
    (defun rhol/switch-window ()
      (interactive)
      (cond ((equal current-prefix-arg nil)
             (other-window 1))
            (t (switch-window))))

    (defun rhol/maximize-window ()
      (interactive)
      (cond ((equal current-prefix-arg nil)
             (maximize-window))
            (t (switch-window-then-maximize))))

    (defun rhol/split-window-below ()
      (interactive)
      (cond ((equal current-prefix-arg nil)
             (split-window-below))
            (t (switch-window-then-split-below current-prefix-arg))))

    (defun rhol/split-window-right ()
      (interactive)
      (cond ((equal current-prefix-arg nil)
             (split-window-right))
            (t (switch-window-then-split-right current-prefix-arg))))

    (defun rhol/delete-window ()
      (interactive)
      (cond ((equal current-prefix-arg nil)
             (delete-window))
            (t (switch-window-then-delete))))


    (global-set-key (kbd "C-x o") 'rhol/switch-window)
    (global-set-key (kbd "C-x 1") 'rhol/maximize-window)
    (global-set-key (kbd "C-x 2") 'rhol/split-window-below)
    (global-set-key (kbd "C-x 3") 'rhol/split-window-right)
    (global-set-key (kbd "C-x 0") 'rhol/delete-window)
...
#+begin_src emacs-lisp
  (use-package org-noter
    :config (setq org-noter-notes-search-path '("~/Dropbox/org-mode/wiki")))
#+end_src
...
Sometimes org-mode built into emacs interferes with melpa
version. Need to force a reload. Do this last to make sure all the
packages are loaded properly.

#+begin_src emacs-lisp
  (let ((current-prefix-arg 4))
    (org-reload)
    (rhol/reload-org-buffers))
#+end_src
* org-roam
** Setup roam
#+begin_src emacs-lisp
    (use-package org-roam
          ...
          :config (setq org-roam-directory (expand-file-name "~/Dropbox/org-mode/roam")))
#+end_src

** Deft Support

#+begin_src emacs-lisp
  (global-set-key (kbd "C-c n d") (lambda ()
                                    (let ((deft-recursive t)
                                          (deft-use-filter-string-for-filename t)
                                          (deft-use-default-extension "org")
...
#+begin_src emacs-lisp
  (use-package org-journal
    ...
    (org-journal-dir (expand-file-name "~/Dropbox/org-mode/roam"))
...
Currently disabled. Requires a billing card because only first 1000
snips are free.
